  ...
  <article class="rounded-3xl border border-slate-200/70 bg-gradient-to-br from-white via-white to-indigo-50/40 p-6 shadow-xl shadow-indigo-100/50 transition dark:border-slate-800/70 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950/70 dark:shadow-none">
    <div class="flex flex-col gap-6">
      ...
      <div class="rounded-2xl border border-slate-200/70 bg-white/80 shadow-md shadow-slate-200/60 dark:border-slate-800/70 dark:bg-slate-900/70">
        ...
        <?php else: ?>
          <div class="hidden overflow-x-auto rounded-2xl md:block">
                  ...
                  <td class="px-4 py-4 align-top text-right">
                    <div class="flex flex-col items-end gap-2 text-xs sm:flex-row sm:items-center sm:justify-end sm:gap-3">
                      <div class="flex flex-wrap items-center justify-start gap-2 sm:justify-end sm:gap-3">
                        ...
                        <?php else: ?>
                          <span class="inline-flex items-center gap-2 rounded-xl border border-slate-200/70 bg-slate-100/70 px-3 py-1.5 text-xs font-semibold text-slate-400 dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-slate-500" title="Proyecto en vista">
              ...
              <article class="space-y-4 rounded-2xl border border-slate-200/70 bg-white/90 p-4 shadow-sm shadow-slate-200/60 dark:border-slate-800/70 dark:bg-slate-900/80">
                <div class="space-y-1">
                  ...
                  <div class="text-sm text-slate-700 dark:text-slate-200">
                    <p class="font-medium">Estudiante: <?= e($project['student_name']); ?></p>
                    <p class="text-[11px] text-slate-400"><?= e($project['student_email']); ?></p>
                  </div>
                  <p class="text-xs font-medium uppercase tracking-wider text-slate-400">Periodo</p>
                ...
                <div class="flex flex-col gap-3 text-xs">
                  <div class="flex flex-wrap items-center gap-2">
                    <?php if (!$isProjectSelected): ?>
                      <a class="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200/70 bg-white/70 px-3 py-1.5 font-semibold text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:bg-indigo-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 dark:border-slate-700/60 dark:bg-slate-900/70 dark:text-slate-200" href="<?= e(url('/dashboard?tab=proyectos&project=' . (int) $project['id'])); ?>">
                        <i data-lucide="eye" class="h-3.5 w-3.5"></i> Ver
                      </a>
                    <?php else: ?>
                      <span class="inline-flex flex-1 items-center justify-center gap-2 rounded-xl border border-slate-200/70 bg-slate-100/70 px-3 py-1.5 font-semibold text-slate-400 dark:border-slate-700/60 dark:bg-slate-800/50 dark:text-slate-500" title="Proyecto en vista">
                        <i data-lucide="focus" class="h-3 w-3"></i> En vista
                      </span>
                    <?php endif; ?>
                    ...
                    <?php if ($isProjectDirector): ?>
                      <button
                        ...
                        data-project-start="<?= e((string) ($project['start_date'] ?? '')); ?>"
                        data-project-end="<?= e((string) ($project['end_date'] ?? ($project['due_date'] ?? ''))); ?>"
                        class="inline-flex items-center gap-2 rounded-xl border border-indigo-200/70 bg-white px-3 py-1.5 font-semibold text-indigo-600 shadow-sm transition hover:-translate-y-0.5 hover:bg-indigo-50 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 dark:border-indigo-900/50 dark:bg-slate-900/60 dark:text-indigo-300 dark:hover:bg-indigo-900/40"
                      >
                        <i data-lucide="pencil" class="h-3.5 w-3.5"></i> Editar
                      </button>
                    <?php endif; ?>
                  ...
                  <?php if ($isProjectDirector): ?>
                    <div class="flex flex-col gap-3">
                      <form method="post" action="<?= e(url('/projects/status')); ?>" class="flex items-center gap-2">
                        ...
                        <button type="submit" class="inline-flex items-center gap-2 rounded-xl bg-gradient-to-r from-indigo-600 to-indigo-500 px-3 py-1.5 font-semibold text-white shadow-md shadow-indigo-200/70 transition hover:-translate-y-0.5 hover:from-indigo-500 hover:to-indigo-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 dark:shadow-none">
                          <i data-lucide="save" class="h-3.5 w-3.5"></i>
                        </button>
                      </form>
